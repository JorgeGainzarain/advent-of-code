...
function processFunction(data) {
    ...
    const nanoseconds = positions.length;


    for (let i = nanoseconds; i > 0; i--) {
        const grid = createGrid(data, i);
        const { path } = bfs(grid);
        ...
        if (path.length !== 0) {
            const lastPosition = positions[i];
...
const correctResults = ['6,1'];
runDay(18, 2, loadData, processFunction, correctResults);